...
def create_product():
    if request.method == 'POST'
        ...
        db.session.commit()
        flash('The product has been created' % name, 'success')
        return render_template(url_for('catalog.product', id=product.id))
...
class ProductApi(Resource):
    ...
    def get(self, id=None, page=1):
        if not id:
            products = Product.query.paginate(page, 10).items #Know what is items
        else:
